#!/bin/sh
# cw149: cw148 (cold warp for kernel #142 -- does the restored session come up
# LIT by itself) PLUS a keyboard probe at the replay point, for task #143:
# "cold-restored session has a live userspace but no input device".
#
# The keyboard is the tc3589x MFD on i2c-0 addr 0x45.  Its chip_init() is
# replayed by tc3589x_resume() whenever pm_device_down != 0, and that resume is
# reached from pm_device_resume() (warp.c:2400) on the resume path -- so the
# re-init *should* happen already.  kbdcheck() is the discriminator:
#   i2cget -f -y 0 0x45 0x80 -> 0x03   i2c controller alive AND chip answers
#   ... 0xF3 (DKBDMSK) = 0x03       chip programmed (keyboard should work)
#                        = 0x00     chip lost its config (resume re-init did
#                                   not run / its i2c writes failed)
#   i2cget error / -EIO             i2c controller itself is dead
# Everything else is cw148, unmodified.
#
...
kl     () { echo "cw149: $*" > /dev/kmsg 2>/dev/null; }
say    () { echo "$*"; kl "$*"; }
